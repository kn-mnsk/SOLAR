...
namespace LoaderGLTF
{
	...
	public:
		...
		inline void debug_glTFAccessorData(std::string attribName, std::string writeDirectory, std::vector<const void*> data, bool debug); // To be coded
		...
		void processPrimitive(int32_t nodeIndex, int32_t meshIndex, int32_t primitiveIndex)
		{
			...
			for each (auto attrib in primitive->attributes)
			{
			...
			for each (auto attrib in primitive->attributes)
			{
				...
				if (attrib.first == "POSITION")
				{
					...
					{// test
						/*int32_t iValue = loaded_gltf->accessors[attrib.second->int32_tValue]->componentType;
						libgltf::EAccessorComponentType eType = libgltf::Int32ToAccessorComponentType(iValue);
						int eTypeSize = libgltf::GSAccessorComponentTypes[(uint32_t)eType].size;
						int eTypeValue = libgltf::GSAccessorComponentTypes[(uint32_t)eType].value;
						position_stream = std::make_shared<libgltf::TAccessorStream<libgltf::TVertexList<eTypeSize, >>>(position_data);*/
					}// end test

					position_stream = std::make_shared<libgltf::TAccessorStream<libgltf::TVertexList<3, float>>>(position_data);

					if (LoadMeshPrimitiveAttributeData(meshIndex, primitiveIndex, attrib.first, position_stream))
					{
					...
					{
						ERROR_EXIT("can not Load Mesh Primitive Attribute Data :" << attrib.first << "Failed");
					};
				...
				else if (attrib.first == "NORMAL")
				{
					...
					if (LoadMeshPrimitiveAttributeData(meshIndex, primitiveIndex, attrib.first, normal_stream))
					{
						// glBufferSubdata params
					...
					{
						ERROR_EXIT("can not Load Mesh Primitive Attribute Data: %s" << attrib.first);
					};
				}
				else if (attrib.first == "TEXCOORD_0")
				{
					...
					if (LoadMeshPrimitiveAttributeData(meshIndex, primitiveIndex, attrib.first, texcoord_0_stream))
					{
						// glBufferSubdata params
				...
				else if (attrib.first == "TANGENT")
				{
					...
					if (LoadMeshPrimitiveAttributeData(meshIndex, primitiveIndex, attrib.first, tangent_stream))
					{
						// glBufferSubdata params
				...
				{
					//ssMsg.clear();
					//ssMsg << "Unrecognized attribute: %s: Failed" << attrib.first;
					ERROR_EXIT("Unrecognized attribute: %s: Failed" << attrib.first);
				}

				// glBufferSubdata params
				//pointer = (void*)AttributesData.at(attrib.first).bufferData.bufferStride;
				index = attrib.second->int32_tValue; // test
				...
				GL_ERROR_CHECK2("glBufferSubData:  Mesh=" << loaded_gltf->nodes[nodeIndex]->name.c_str() << ", Vertex=" << attrib.first.c_str() << ", Index=" << index << "bytelengthAll = offset + size-> " << byteLengthInAll << " = " << offset << " + " << sizeptr);
				...
				{// debug
					...
					if (folderName == "NA") // e.g. == "Phobos" to Log) 
					{
						WIN_LOG("glVertexAttribPointer: " << folderName.c_str() << ", Mesh = " << loaded_gltf->nodes[nodeIndex]->name.c_str() << ", Attribute = " << attrib.first.c_str() << ", Index = " << index << ", Target=" << target << ":" << Tools::glEnumToString(target) << ", bytelengthAll = offset + sizeptr-> " << byteLengthInAll << " = " << offset << " + " << sizeptr << ", Component Size=" << sizeint << ", Type=" << type << ":" << Tools::glEnumToString(type) << ", Stride=" << stride << ", Pointer=" << (int)pointer);
					}
				}
