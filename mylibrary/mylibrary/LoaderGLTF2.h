...
#define Debug_Folder "NA" // debug spefic folder including glTF model
...
namespace LoaderGLTF2
{
	//https://libgltf.readthedocs.io/en/latest/

	template<size_t s, typename T>
	class PrimitiveStream : public libgltf::TVertexList<s, T>
	{
	public:
		typedef libgltf::TAccessorStream<libgltf::TVertexList<s, T> > PStream;
		typedef libgltf::TVertexList<s, T> PData;

	public:
		std::shared_ptr<PStream> Stream()
		{
			std::shared_ptr<PStream> ptr = std::make_shared<SStream<s, T>>(streamdata);
			return ptr;
		}
	...
	template<size_t s, typename T>
	void debug_glTFAccessorData(std::string aName, std::string writeDirectory, libgltf::TVertexList<s, T> data, bool debug)
	{
		if (!debug) return;
		...
		std::string headline;
		std::string line;
		std::string fileName = writeDirectory + "/glTF_debug_" + aName + ".csv";
		...
		for (auto& d : data)
		{
			...
			{
				headline.append(aName + std::to_string(i) + delimiter);
			}
			headline.append("\n");
			size = headline.size();
	...
	public:
		...
		std::map<std::string, libgltf::SAccessorData> IndicesData{};
		std::map<std::string, libgltf::SAccessorData> PrimitiveData{};

		std::shared_ptr<libgltf::IglTFLoader> gltf_loader; // gltf loader
		libgltf::SGlTF* loaded_gltf{}; // The root object for a glTF asset.
		...
		std::map<std::string, int> vertexShaderAttribIndex {
			{"POSITION", 0},
			{"NORMAL", 1},
			{"TEXCOORD_0", 2},
			{"TANGENT", 3},
		...
		std::string glTFModelFolder{};
		std::string fileName{};
		bool gammaCorrection;
		GLenum pMode{}; // The topology type of primitives to render.
		std::vector<std::map<std::string, std::shared_ptr<libgltf::SGlTFId>>> morphtargets{}; // An array of morph targets.
		...
		void drawModelByNode(int32_t nodeIndex, Shader::Shader* shaderProg)
		{
			...
			GLenum type = libgltf::GSAccessorComponentTypes[(uint32_t)IndicesData.at("INDICES").componentType].value;
			//GLenum type = GL_UNSIGNED_SHORT;
			
			{// debug : draw only saturn ring: currently ring is not drawn WHY??
				if (glTFModelFolder == Debug_Folder) // Saturn planet
				{
					switch(nodeIndex)
					{
					case 0: // Saturn planet
					{
						break;// skip glDrawElements 
					}
					case 1: // RingTop
					case 2: // Ringbottom
					{
						glDrawElements(pMode, count, type, 0);
						WIN_LOG_DEBUG(glTFModelFolder.c_str() << " ; " << loaded_gltf->nodes[nodeIndex]->name << " : glDrawElements(" << Tools::glEnumToString(pMode) << ", " << count << ", " << Tools::glEnumToString(type) << ",   0);  mappingMethod=" << (int)nodeMapInfo[nodeIndex].mappingMethod);
						break;
					}
					}
				}
			...
			//glDrawElements(pMode, count, type, 0); 
			GL_ERROR_CHECK2("mode=" << pMode << "count=" << count << "type=<< ype");
			{// debug
				if (glTFModelFolder == Debug_Folder) // e.g. == "Phobos" to Log) 
				{
					WIN_LOG_DEBUG(glTFModelFolder.c_str() << ": " << "glDrawElements(" << Tools::glEnumToString(pMode) << ", " << count << ", " << Tools::glEnumToString(type) << ",   0);  mappingMethod=" << (int)nodeMapInfo[nodeIndex].mappingMethod);
				}
			}
			glBindVertexArray(0);
		...
		std::shared_ptr<libgltf::IglTFLoader> Loader(const std::string& path)
		{
			...
			glTFModelFolder = directory.substr(directory.find_last_of('/') + 1, string::npos);
		...
		bool LoadMeshPrimitiveIndicesData(size_t _mesh_index, size_t _primitive_index, std::shared_ptr<libgltf::IAccessorStream> _accessor_stream)
		{
			...
			dataURI = tinygltf2::IsDataURI(buffer->uri);
			if (dataURI)
			{
				tinygltf2::DecodeDataURI(&out, mime_type, buffer->uri, buffer->byteLength, false);
			...
			IndicesData.emplace("INDICES", accessordata);
		...
		bool LoadImageData(size_t _index, std::vector<uint8_t>& _data, std::string& _type, GLuint& _textureId)
		{
			...
			{// read external buffer binary data file
				std::string filepath = directory + "/" + buffer->uri;
			...
			{// debug   case only for external file
				if (false)
				{
		...
		void processPrimitive(int32_t nodeIndex, int32_t meshIndex, int32_t primitiveIndex)
		{
			...
			pMode = primitive->mode;  // for debug
			morphtargets = primitive->targets;

			// for each attribute
			std::shared_ptr<libgltf::SAccessor> accessor{};
			std::shared_ptr<libgltf::SBufferView> bufferview{};
			//std::shared_ptr<libgltf::SAccessorData> saccessordata{};
			//std::shared_ptr<libgltf::SBufferData> sbufferdata{};

			int32_t attribByteLength{ 0 };

			std::map<GLuint, float> bufferDataOffset{};
			for (auto& attrib : primitive->attributes)
			{
				accessor = loaded_gltf->accessors[attrib.second->int32_tValue];
				bufferview = loaded_gltf->bufferViews[accessor->bufferView->int32_tValue];
				attribByteLength += bufferview->byteLength;

				bufferDataOffset.emplace(attrib.second->int32_tValue, bufferview->byteOffset);
			}
			...
			glBufferData(target, attribByteLength, 0, GL_DYNAMIC_DRAW);

			GL_ERROR_CHECK2("glBufferData: Mesh = " << loaded_gltf->nodes[nodeIndex]->name.c_str() << ", BytelengthAll = " << attribByteLength);

			// attributes
			for (auto& attrib : primitive->attributes)
			{
				if ((attrib.first == "POSITION") || (attrib.first == "NORMAL") || (attrib.first == "TANGENT") || (attrib.first == "BITANGENT"))
				{
					vec3_data.clear();
					bool result = LoadMeshPrimitiveAttributeData(meshIndex, primitiveIndex, attrib.first, vec3_stream);
					if (!result)
					{
						ERROR_EXIT("can not Load Mesh Primitive Attribute Data: " << attrib.first << " of " << glTFModelFolder << ", meshIndex=" << meshIndex);
					}
					data = vec3_data.Data();
				}
				...
				{
					vec2_data.clear();
					bool result = LoadMeshPrimitiveAttributeData(meshIndex, primitiveIndex, attrib.first, vec2_stream);
					if (!result)
					{
						ERROR_EXIT("can not Load Mesh Primitive Attribute Data: " << attrib.first << " of " << glTFModelFolder << ", meshIndex=" << meshIndex);
					}
				...
				{
					ERROR_EXIT("Unrecognized attribute Failed : " << "glTF=" << attrib.first << ",  nodeIndex=" << nodeIndex << ", meshIndex=" << meshIndex << "primitiveIndex=" << primitiveIndex << ", attrib name=" << attrib.first);
				}

				// glBufferSubdata params
				//pointer = (void*)AttributesData.at(attrib.first).bufferData.bufferStride;
				//index = attrib.second->int32_tValue; // Wrong
				index = vertexShaderAttribIndex[attrib.first];
				offset = bufferDataOffset.at(attrib.second->int32_tValue);
				pointer = (void*)offset;
				sizeptr = AttributesData.at(attrib.first).bufferData.bufferSize;

				// glVertexAttribPointer params
				sizeint = libgltf::DimensionOfAccessorType(AttributesData.at(attrib.first).type);
				type = libgltf::GSAccessorComponentTypes[(uint8_t)AttributesData.at(attrib.first).componentType].value;
				stride = sizeint * SizeOfAccessorComponentType(AttributesData.at(attrib.first).componentType);

				{// debug
					if (glTFModelFolder == Debug_Folder) // debug specific star's folder
					{
						debug_glTFAccessorData<3, float>(loaded_gltf->nodes[nodeIndex]->name + "_" + std::to_string(index) + "_" +attrib.first, directory, vec3_data, true);
					}
				}

				glBufferSubData(target, offset, sizeptr, data);
				GL_ERROR_CHECK2("glBufferSubData:  Mesh=" << loaded_gltf->nodes[nodeIndex]->name.c_str() << ", Vertex=" << attrib.first.c_str() << ", Index=" << index << "bytelengthAll = offset + size-> " << attribByteLength << " = " << offset << " + " << sizeptr);

				glEnableVertexAttribArray(index);
				GL_ERROR_CHECK2("glEnableVertexAttribArray: Mesh=" << loaded_gltf->nodes[nodeIndex]->name.c_str() << ", Vertex=" << attrib.first.c_str() << ", Index=" << index);

				glVertexAttribPointer(index, sizeint, type, GL_FALSE, stride, pointer);
				GL_ERROR_CHECK2("glVertexAttribPointer: Mesh=" << loaded_gltf->nodes[nodeIndex]->name.c_str() << ", Vertex=" << attrib.first.c_str() << ", Index=" << index);
				{// debug
					if (glTFModelFolder == Debug_Folder) // e.g. == "Phobos" to Log) 
					{
						WIN_LOG_DEBUG("glVertexAttribPointer: " << glTFModelFolder.c_str() << ", Mesh = " << loaded_gltf->nodes[nodeIndex]->name.c_str() << ", Attribute = " << attrib.first.c_str() << ", Index = " << index << ", Target=" << target << ":" << Tools::glEnumToString(target) << ", bytelengthAll = offset + sizeptr-> " << attribByteLength << " = " << offset << " + " << sizeptr << ", Component Size=" << sizeint << ", Type=" << type << ":" << Tools::glEnumToString(type) << ", Stride=" << stride << ", Pointer=" << (int)pointer);
					}
				}
			...
			// indices
			scalar_data.clear();
			bool result = LoadMeshPrimitiveIndicesData(meshIndex, primitiveIndex, scalar_stream);
			if (!result)
			{
				ERROR_EXIT("can not Load Mesh Primitive Attribute Data: " << "INDICES" << " of " << glTFModelFolder << ", meshIndex=" << meshIndex);
			}

			target = GL_ELEMENT_ARRAY_BUFFER;
			sizeptr = IndicesData.at("INDICES").bufferData.bufferSize;
			data = scalar_data.Data();

			{// debug
				if (glTFModelFolder == Debug_Folder)
				{
					debug_glTFAccessorData<3, float>(loaded_gltf->nodes[nodeIndex]->name + std::string("_INDICES"), directory, vec3_data, true);
				}
			}

			//nodeMapInfo[nodeIndex].indicesCount = indices_data.size();
			nodeMapInfo[nodeIndex].indicesCount = scalar_data.size();
				...
				if (primitive->targets.size() > 0)
				{
					for each (auto target in primitive->targets)
					{
						for each (auto map in target)
						{
							...
							WIN_LOG("Debug Morph Targets:  Name="  << name << ", Value=" << value << "glTF Model=" << glTFModelFolder);
						}
					}
				}
			}
		...
		void processMaterial(int32_t nodeIndex, int32_t materialIndex)
		{
			...
			{// albedo,  and metallic&roughness
				...
				else if ((material->pbrMetallicRoughness->baseColorTexture != nullptr) && (material->pbrMetallicRoughness->metallicRoughnessTexture == nullptr))
				{
					...
					result = LoadImageData(textureIdx, data, imageMimeType, nodeMapInfo[nodeIndex].baseColorTexture.Id);
					...
					{// debug : draw only saturn ring: currently ring is not drawn WHY??
						if (glTFModelFolder == Debug_Folder) // Saturn planet
						{
							...
							case 2: // Ringbottom
							{
								WIN_LOG_DEBUG("Load Material Image Data: Texture Index=" << textureIdx << ", Image mimie type=" << imageMimeType << ", base color Texture Id=" << nodeMapInfo[nodeIndex].baseColorTexture.Id << "Node Index = " << nodeIndex << ", Material Index = " << materialIndex << ", glTF Model = " << glTFModelFolder);
								break;
							}
							}
						}
				...
				else if ((material->pbrMetallicRoughness->baseColorTexture == nullptr) && (material->pbrMetallicRoughness->metallicRoughnessTexture != nullptr))
				{
					...
					std::vector<uint8_t> data{};
					std::string imageMimeType{};
					result = LoadImageData(imageIdx, data, imageMimeType, nodeMapInfo[nodeIndex].metallicRoughnessTexture.Id);
					if (!result) ERROR_EXIT("");
					...
					{// debug : draw only saturn ring: currently ring is not drawn WHY??
						if (glTFModelFolder == Debug_Folder) // Saturn planet
						{
							...
							case 2: // Ringbottom
							{
								WIN_LOG_DEBUG("Load Material Image Data: Image Index=" << imageIdx << ", Image mimie type=" << imageMimeType << ", metallic  roughness Texture Id=" << nodeMapInfo[nodeIndex].metallicRoughnessTexture.Id << "Node Index = " << nodeIndex << ", Material Index = " << materialIndex << ", glTF Model = " << glTFModelFolder);
								break;
							}
							}
						}
				...
				else if ((material->pbrMetallicRoughness->baseColorTexture != nullptr) && (material->pbrMetallicRoughness->metallicRoughnessTexture != nullptr))
				{
					...
					std::vector<uint8_t> data{};
					std::string imageMimeType{};
					result = LoadImageData(imageIdx, data, imageMimeType, nodeMapInfo[nodeIndex].baseColorTexture.Id);
					if (!result) ERROR_EXIT("");
					...
					{// debug : draw only saturn ring: currently ring is not drawn WHY??
						if (glTFModelFolder == Debug_Folder) // Saturn planet
						{
							...
							case 2: // Ringbottom
							{
								WIN_LOG_DEBUG("Load Material Image Data: image Index=" << imageIdx << ", Image mimie type=" << imageMimeType << ", base color Texture Id=" << nodeMapInfo[nodeIndex].baseColorTexture.Id << "Node Index = " << nodeIndex << ", Material Index = " << materialIndex << ", glTF Model = " << glTFModelFolder);
								break;
							}
							}
						}
					...
					{// debug : draw only saturn ring: currently ring is not drawn WHY??
						if (glTFModelFolder == Debug_Folder) // Saturn planet
						{
							...
							case 2: // Ringbottom
							{
								WIN_LOG_DEBUG("Load Material Image Data: image Index=" << imageIdx << ", Image mimie type=" << imageMimeType << ", mettalic rougness Texture Id=" << nodeMapInfo[nodeIndex].metallicRoughnessTexture.Id << "Node Index = " << nodeIndex << ", Material Index = " << materialIndex << ", glTF Model = " << glTFModelFolder);
								break;
							}
							}
						}
			...
			{// The tangent space normal texture.
				...
				else
				{
					...
					std::vector<uint8_t> data{};
					std::string imageMimeType{};
					result = LoadImageData(imageIdx, data, imageMimeType, nodeMapInfo[nodeIndex].normalTexture.Id);
					if (!result) ERROR_EXIT("");
					...
					{// debug : draw only saturn ring: currently ring is not drawn WHY??
						if (glTFModelFolder == Debug_Folder) // Saturn planet
						{
							...
							case 2: // Ringbottom
							{
								WIN_LOG_DEBUG("Load Material Image Data: Image Index=" << imageIdx << ", Image mimie type=" << imageMimeType << ", Normal Texture Id=" << nodeMapInfo[nodeIndex].normalTexture.Id << "Node Index = " << nodeIndex << ", Material Index = " << materialIndex << ", glTF Model = " << glTFModelFolder);
								break;
							}
							}
						}
			...
			{// The occlusion texture.
				...
				else
				{
					...
					std::vector<uint8_t> data{};
					std::string imageMimeType{};
					result = LoadImageData(imageIdx, data, imageMimeType, nodeMapInfo[nodeIndex].occlusionTexture.Id);
					if (!result) ERROR_EXIT("");
					...
					{// debug : draw only saturn ring: currently ring is not drawn WHY??
						if (glTFModelFolder == Debug_Folder) // Saturn planet
						{
							...
							case 2: // Ringbottom
							{
								WIN_LOG_DEBUG("can not Load Material Image Data: Image Index=" << imageIdx << ", Image mimie type=" << imageMimeType << ", Occulusion Texture Id=" << nodeMapInfo[nodeIndex].occlusionTexture.Id << "Node Index = " << nodeIndex << ", Material Index = " << materialIndex << ", glTF Model = " << glTFModelFolder);
								break;
							}
							}
						}
			...
			{// The emissive texture.
				...
				else
				{
					...
					std::vector<uint8_t> data{};
					std::string imageMimeType{};
					result = LoadImageData(imageIdx, data, imageMimeType, nodeMapInfo[nodeIndex].emissiveTexture.Id);
					if (!result) ERROR_EXIT("");
					...
					{// debug : draw only saturn ring: currently ring is not drawn WHY??
						if (glTFModelFolder == Debug_Folder) // Saturn planet
						{
							...
							case 2: // Ringbottom
							{
								WIN_LOG_DEBUG("Load Material Image Data: Image Index=" << imageIdx << ", Image mimie type=" << imageMimeType << ", Emmissive Texture Id=" << nodeMapInfo[nodeIndex].emissiveTexture.Id << "Node Index = " << nodeIndex << ", Material Index = " << materialIndex << ", glTF Model = " << glTFModelFolder);
								break;
							}
							}
						}
	...
	private:
		...
		libgltf::TVertexList<1, unsigned short> scalar_data{};
		libgltf::TVertexList<2, float> vec2_data{};
		libgltf::TVertexList<3, float> vec3_data{};
		...
		std::shared_ptr<libgltf::TAccessorStream<libgltf::TVertexList<1, unsigned short> > > scalar_stream = std::make_shared<libgltf::TAccessorStream<libgltf::TVertexList<1, unsigned short>>>(scalar_data);
		std::shared_ptr<libgltf::TAccessorStream<libgltf::TVertexList<2, float> > > vec2_stream = std::make_shared<libgltf::TAccessorStream<libgltf::TVertexList<2, float>>>(vec2_data);
		std::shared_ptr<libgltf::TAccessorStream<libgltf::TVertexList<3, float> > > vec3_stream = std::make_shared<libgltf::TAccessorStream<libgltf::TVertexList<3, float>>>(vec3_data);
